...
class AmazonMwsClient {
    ...
    constructor(options) {
        ...
        this.meta = opts.meta;
        this.reSignService = opts.reSignService;
        _.merge(this, require('./api')(this));
    }

    async request(req, q, meta) {
        const api = req.api;
        const action = req.action;
        let uri = `https://${this.host}${this.base}${api.path}`;
        const requestOpts = {};
        const query = _.clone(q);
        const {reSignService} = this;
        ...
        if(reSignService){
            headers['MWSProxy-Return-Type'] = 're-signed-request';
        }
        ...
        if(reSignService){
            let response = await request.postAsync(uri, requestOpts);
            const redirect = JSON.parse(response.body);
            uri = redirect.uri;
            requestOpts.headers = redirect.headers;
            requestOpts[api.upload ? 'qs' : 'form'] = redirect.body;
        }
     ...
     */
    call(req, q, meta) {
        //console.log(req, q, meta);
        return this.request(req, q, meta).then((response) => {
            //console.log(response.statusCode, meta);
            ...
            if (body.slice(0, 5) !== '<?xml' &&  body.indexOf('xmlns=') < 0) {
