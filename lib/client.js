...
class AmazonMwsClient {
    ...
    call(req, q, meta) {
        return this.request(req, q, meta).then((response) => {
            ...
            const bodyStart = body.slice(0, 5);

            // Need the special check because GetMyFeesEstimate doesn't have an xml tag like it should
            if ((bodyStart !== '<?xml') && (bodyStart !== '<GetM') && (bodyStart !== '<List')) {
