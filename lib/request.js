...
class AmazonMwsRequest {
    ...
    set(paramName, value) {
        ...
        const param = _.get(this.params, paramName);

        if(!param){
            throw Error(`Invalid param: ${paramName}`);
        }

        let queryKey = param.name;

        if(paramName !== param.name){
            const parts = paramName.split(".");
            const prefix = parts.slice(0, parts.length-1).join(".");
            queryKey = prefix === '' ?  param.name : `${prefix}.${param.name}`;
        }

        if (!param.keyValue && _.isObject(value) && !Array.isArray(value)) {
            return this.setMultiple(value, paramName);
        }
        ...
        if (value !== null && value !== undefined) {
            ...
            if (param.list) {
                ...
                _.forEach(values, (value, i) => {
                    this.values[`${queryKey}.${i + 1}`] = value;
                });
            }else if(param.keyValue){
                const keys = Object.keys(value);
                _.forEach(keys, (key, i) => {
                    this.values[`${queryKey}.member.${i + 1}.Key`] = key;
                    this.values[`${queryKey}.member.${i + 1}.Value`] = value[key];
                });
            }else {
                this.values[queryKey] = getValue(value);
            }
        }
